...
1. 입력받는 문자열을 분리해서 list에 넣는다.
2. list에 p나 P가 있다면 cnt_p에 1을 더한다.
3. list에 y나 Y가 있다면 cnt_y에 1을 더한다.
4. cnt_p와 cnt_y가 같거나, 문제 조건에 따라 둘다 0일경우 True 반환
5. 둘다 다른 숫자라면 False를 반환한다.
